...
class RecappWindow(Gtk.ApplicationWindow):
    ...
    def __init__(self, **kwargs):
        ...
        if self.displayServer == "wayland":
            ...
            self.video_str = "gst-launch-1.0 ximagesrc use-damage=0 show-pointer={} ! video/x-raw,framerate={}/1 ! queue ! videoscale ! videoconvert ! {} ! queue ! webmux name=mux ! queue ! filesink location='{}'.webm"
    ...
    def on__record_button_clicked(self, button):
        ...
        if self.displayServer == "wayland":
            ...
            if self.coordinateMode == True:
                video_str = "gst-launch-1.0 ximagesrc show-pointer={} " +self.coordinateArea +" ! video/x-raw,framerate={}/1 ! queue ! videoscale ! videoconvert ! {} ! queue ! webmux name=mux ! queue ! filesink location='{}'.webm"
                print(video_str)
                if self.recordSoundOn == True:
